...
type BudgetsAction =
  ...
  | { type: 'UPDATE_ENTRY'; entryId: string; field: string; value: string | null }
  | { type: 'DELETE_ENTRY'; entryId: string }
  | { type: 'UPDATE_INCOME'; budgetId: string; income: string }

function budgetsReducer(budgets: SerializedBudget[], action: BudgetsAction): SerializedBudget[] {
  switch (action.type) {
    case 'ADD_BUDGET':
      return [...budgets, action.budget].sort((a, b) => a.month - b.month)
    case 'ADD_ENTRY':
      return budgets.map(b =>
        b.id === action.budgetId ? { ...b, entries: [...b.entries, action.entry] } : b,
      )
    case 'UPDATE_ENTRY':
      return budgets.map(b => ({
        ...b,
    ...
    case 'UPDATE_INCOME':
      return budgets.map(b => (b.id === action.budgetId ? { ...b, income: action.income } : b))
    default:
      return budgets
  }
}

export function BoardGridClient({ boardId, boardName, initialBudgets, year }: Props) {
  const [budgets, setBudgets] = useState<SerializedBudget[]>(initialBudgets)
  const [optimisticBudgets, addOptimistic] = useOptimistic(budgets, budgetsReducer)
  const [showLoans, setShowLoans] = useState(false)
  const [showChart, setShowChart] = useState(false)

  function handleBudgetCreated(budget: SerializedBudget) {
    setBudgets(prev => [...prev, budget].sort((a, b) => a.month - b.month))
  }

  async function handleAddEntry(budgetId: string) {
    ...
    const res = await fetch(`/api/boards/${boardId}/budgets/${budgetId}/entries`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ categoryName: '', amount: '0', type: 'expense' }),
    })
    ...
    if (res.ok) {
      ...
      setBudgets(prev =>
        prev.map(b =>
          b.id === budgetId
            ? { ...b, entries: [...b.entries.filter(e => e.id !== tempEntry.id), created] }
            : b,
        ),
      )
    } else {
      setBudgets(prev =>
        prev.map(b =>
          b.id === budgetId
            ? { ...b, entries: b.entries.filter(e => e.id !== tempEntry.id) }
            : b,
        ),
      )
    }
  ...
  async function handleUpdateEntry(entryId: string, field: string, value: string | null) {
    const prevBudgets = budgets
    setBudgets(prev =>
      prev.map(b => ({
        ...b,
        entries: b.entries.map(e => (e.id === entryId ? { ...e, [field]: value } : e)),
      })),
    )
    startTransition(() => {
      addOptimistic({ type: 'UPDATE_ENTRY', entryId, field, value })
    })
  ...
  async function handleDeleteEntry(entryId: string) {
    const prevBudgets = budgets
    setBudgets(prev =>
      prev.map(b => ({ ...b, entries: b.entries.filter(e => e.id !== entryId) })),
    )
    const res = await fetch(`/api/entries/${entryId}`, { method: 'DELETE' })
    if (!res.ok) setBudgets(prevBudgets)
  }

  async function handleIncomeUpdate(budgetId: string, income: string) {
    const prevBudgets = budgets
    setBudgets(prev => prev.map(b => (b.id === budgetId ? { ...b, income } : b)))
    startTransition(() => {
      addOptimistic({ type: 'UPDATE_INCOME', budgetId, income })
    })
  ...
  return (
    <div className="flex h-full flex-col overflow-hidden">
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        {/* Loans sidebar */}
        {showLoans && (
          <div className="w-72 flex-shrink-0 overflow-y-auto border-l border-gray-200 bg-gray-50">
      ...
      {/* Yearly chart */}
      {showChart && (
        <div className="flex-shrink-0 overflow-y-auto border-t border-gray-200">
